 ...
 * 强调/代码/标题并破坏列表结构（Kimi PR#30 P2）。换行同理折叠为空格，防
 * 断行破坏列表续行缩进（Kimi PR#30 minor）。备注为本地确定性文案，不经此转义。
 */
const mdEscape = (s: string): string => s.replace(/([\\`*_[\]#])/g, '\\$1').replace(/\n/g, ' ');
...
export function buildMarkdown(result: GenerateResult): string {
  ...
  for (const [gi, g] of buildDisplayGroups(result).entries()) {
    ...
    for (const row of g.items) {
      ...
      // pad 与编号宽度一致时，缩进相对列表标记为 0——CommonMark 列表项内
      // 该行是 blockquote 而非缩进代码块（Kimi PR#30 该项经 marked 实测驳回）
