...
export function Step3(props: Step3Props): React.JSX.Element {
  const { result } = props;
  // 逐条覆盖全局开关（点开/收起单条后仍可被全局切换重置——以 allOpen 变更清空）
  const [overrides, setOverrides] = useState<Record<number, boolean>>({});
  const [lastAllOpen, setLastAllOpen] = useState(props.allOpen);
  if (lastAllOpen !== props.allOpen) {
    setLastAllOpen(props.allOpen);
    setOverrides({});
  }
      ...
      <div className="doc">
        ...
        {groups.map((g, gi) => (
          <div className="cat" key={g.category}>
            ...
            {g.items.map((row) => {
              const open = overrides[row.no] ?? props.allOpen;
                  ...
                  {row.item.zh !== undefined && (
                    <details
                      ...
                      onToggle={(e) => {
                        ...
                        if (nowOpen !== open) {
                          setOverrides((prev) => ({ ...prev, [row.no]: nowOpen }));
                        }
                      }}
                    >
