...
2. **隐私**：申请人姓名、护照号等个人信息只在本地拼接（标题等），**永不发送给任何 AI provider**（Claude/OpenAI/MiMo 一视同仁）。发给 AI 的只能是官网公开的清单文本。
...
- 三家 provider，设置页添加 key、多选启用、按用户排序 fallback：
  ...
  - **MiMo（小米）**：OpenAI/Anthropic 双协议兼容 API，复用 `openai` SDK + baseURL 覆写；模型 `mimo-v2.5-pro` / `mimo-v2.5`。**注意 MiMo 是 Token Plan 套餐计费**——配额耗尽错误必须触发 fallback 并在 UI 提示。
...
- 术语表（CoE=入学确认书、OSHC=海外学生健康保险、GS=真实学生要求、CRICOS、Form 956/956A 等）三家共用同一份 prompt 模板，保证切换 provider 后术语一致；Claude 侧加 prompt caching。
