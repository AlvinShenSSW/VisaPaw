...
**VisaPaw 的目标**：做一个 Mac 桌面 App（套壳架构，参照 BookingPro / TaskPaw），输入申请人信息后一键完成——

1. 实时检索官网最新材料清单（保证时效性，不做静态快照）；
2. 自动翻译成中文（术语准确、双语可对照）；
3. 按材料类别整理（个人身份、教育工作背景、品行等）；
4. 每项材料注入合规备注（扫描件要求、翻译/公证要求）；
5. 生成带标题的交付文档：`〈姓名〉澳大利亚学生签证（Subclass 500）申请材料清单`。
...
```
〈姓名〉澳大利亚学生签证（Subclass 500）申请材料清单
